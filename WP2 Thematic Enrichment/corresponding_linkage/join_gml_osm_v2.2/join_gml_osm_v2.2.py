...
def spatial_join(osm_file_path,gml_file_path,uni_crs,tolerance,output_path):
    ...
    dict_osm_neigh_origin = {}
    dict_gml_neigh_origin = {}
    for key in dict_osm_neigh:
        if len(dict_osm_neigh[key]) < 1:
            continue
        for i in dict_osm_neigh[key]:
            if len(i)<1:
                continue
            if i in dict_osm_neigh_origin.keys():
                dict_osm_neigh_origin[i].append(key)
            else:
                dict_osm_neigh_origin[i] = [key]
    for key in dict_gml_neigh:
        if len(dict_gml_neigh[key]) < 1:
            continue
        for i in dict_gml_neigh[key]:
            if len(i)<1:
                continue
            if i in dict_gml_neigh_origin.keys():
                dict_gml_neigh_origin[i].append(key)
            else:
                dict_gml_neigh_origin[i] = [key]
    ...
    index_gml4 = pd.DataFrame.from_dict(dict_gml_neigh_origin,orient='index')
    index_gml4.rename(columns=lambda x: 'belong_' + str(x), inplace=True)
    index_gml = pd.concat([index_gml2,index_gml,index_gml3,index_gml4],sort = False, axis = 1)
    ...
    index_osm4 = pd.DataFrame.from_dict(dict_osm_neigh_origin,orient='index')
    index_osm4.rename(columns=lambda x: 'belong_' + str(x), inplace=True)
    index_osm = pd.concat([index_osm2,index_osm,index_osm3,index_osm4],sort = False, axis = 1)
